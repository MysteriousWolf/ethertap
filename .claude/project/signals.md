...
- GUI: `nih-plug-iced` (stateful widget API, fixed 360×280 window)
...
|--------|------------|-----------|--------|
| core | `src/lib.rs`, `src/params.rs`, `src/bin/` | Plugin struct, `process()` RT loop, BPM settle, params, sync dispatch | signals/core.md |
...
| editor | `src/editor.rs` | Iced GUI: LEDs, sync controls, FX slot/device pickers, telemetry display | signals/editor.md |
...
- **Atomic bit-packing convention:** `f32` values shared across threads stored as `u32` via `f32::to_bits`/`f32::from_bits` (BPM, hardware delay float).
