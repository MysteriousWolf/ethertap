...
EtherTap is a zero-audio VST3 plugin (nih-plug, Rust) that bridges DAW BPM to Behringer X32 / Midas M32 delay times via UDP/OSC. The `process()` audio callback dispatches sync commands to the network worker via lock-free channels; it never allocates, blocks, or locks a contended mutex. Sync can fire on BPM settle (500 ms debounce), on every beat, or only on manual trigger.

## Artifacts

- `src/lib.rs` — plugin struct (`EtherTap`), `Plugin` impl, `process()` audio callback, BPM settle state machine, quantised Hard Reset scheduler, reconnect-sync logic
- `src/params.rs` — `EtherTapParams` (VST3 param block), `SyncMode` enum, persisted config fields
...
- `f32` BPM and delay float are stored in atomics as `u32` bit patterns via `f32::to_bits` / `f32::from_bits`.
...
- Default params: target IP `192.168.1.100`, port `10023`, FX slot 1, rate sync = OnChange, phase sync = Manual.
